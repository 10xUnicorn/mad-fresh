...
export async function triggerTestEmails(to: string) {
  try {
    ...
    return { success: true, to, total: results.length, sent, failed, results };
  } catch (err) {
    return {
      success: false,
      error: err instanceof Error ? err.message : "Unknown error",
...
export async function triggerEventReminders() {
  ...
  const BASE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://mad-fresh.vercel.app";
  ...
  try {
    const res = await fetch(`${BASE_URL}/api/send-event-reminders`, {
      method: "POST",
      headers: {
